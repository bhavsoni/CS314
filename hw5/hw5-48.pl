...
xsselect(V, [H|T], [H|V1]) :- H<V, sselect(V, T, V1).
xsselect(V, [V|T], [V|T]).
xsselect(V, [H|T], [V,H|T]) :- H > V.
xsselect(V, [], [V]).
...
sselect(ELEMENT, [], [ELEMENT]).

sselect(ELEMENT, [HEAD|TAIL], [ELEMENT|LIST]) :-
   ELEMENT =< HEAD, sselect(HEAD, TAIL, LIST).

sselect(ELEMENT, [HEAD|TAIL], [HEAD|LIST]) :-
